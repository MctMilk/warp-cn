...
- [x] 8.4.1 `app/src/terminal/view/ssh_remote_server_failed_banner.rs` —— 上游"功能更丰富"版本回归遗留：3 titles + 3 descs + 1 error line 接回 `t!()`，新增 `terminal-ssh-failed-banner-*` 命名空间（en + zh-CN 各 7 key），同步删除孤立 `terminal-ssh-failed-{title,desc}`；error 行用 Fluent 占位符 `{$detail}`
- [ ] 8.5 follow-up：`qa-zh-cn-manual-regression` 提案承接所有真机视觉验证任务（1.19 / 3.9 / 4.7 / 5.13 / 6.12 / 7.3-7.6 / 8.4.1 SSH banner 三种 failure kind），三平台分别归档
